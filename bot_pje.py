...
class SessaoNavegador:
    ...
    def _abrir(self):
        self.navegador = self.p.chromium.launch(
            ...
            # --disable-http2: alguns tribunais (ex: TRF3) retornam
            # ERR_HTTP2_PROTOCOL_ERROR de forma consistente com HTTP/2
            # no Chromium. Forçar HTTP/1.1 evita esse erro.
            args=["--disable-http2"],
        )
...
HEADLESS = True
ARQUIVO_SESSAO = "sessao_pje.json"
...
# ============================================================
def verificar_e_aguardar_cloudflare(sessao, timeout_segundos=60):
    """
    No servidor não há humano para resolver o desafio anti-bot.
    Só espera passivamente por até timeout_segundos — se o texto de
    verificação não sumir sozinho nesse tempo, levanta um erro
    (o tribunal fica marcado como "erro" naquela rodada e tenta de
    novo na próxima).
    """
    indicadores = [
    ...
    print()
    print("==========================================")
    print(" VERIFICAÇÃO ANTI-BOT DETECTADA (aguardando resolver sozinha)")
    print("==========================================")
    ...
    raise RuntimeError(
        "Verificação anti-bot não foi resolvida automaticamente "
        f"dentro de {timeout_segundos}s (sem humano disponível no servidor)."
    )
...
# ============================================================
# FUNÇÃO — LOGIN AUTOMÁTICO (sem certificado — só usuário/senha/2FA)
# ============================================================
def fazer_login(sessao, tribunal):
    ...
    if "sso.cloud.pje.jus.br" not in sessao.pagina.url:
        print("Sessão SSO já ativa — login automático (sem senha).")
        return True
...
def abrir_tela_de_consulta(sessao, tribunal):
    # Alguns tribunais (TJPI, TJRO) demoram mais para carregar essa
    # tela — timeout generoso para não desistir cedo demais. O
    # retry de rede já cobre falhas mais rápidas.
...
with sync_playwright() as p:
            ...
            for tribunal in TRIBUNAIS:
                ...
                if status_bots.esta_pausado_manualmente(nome_tribunal):
                    ...
                    status_bots.atualizar_status(
                        nome_tribunal, NOME_DO_GRUPO, "pausado"
                    )
                    continue
                ...
                try:
                    ...
                    status_bots.atualizar_status(
                        nome_tribunal, NOME_DO_GRUPO, "erro",
                        detalhe_erro=f"Falha ao abrir tela de consulta: {erro}"
                    )
                ...
                for classe in classes_deste_tribunal:
                    ...
                    try:
                        ...
                        erro_na_captacao = str(erro)

                if erro_na_captacao:
                    status_bots.atualizar_status(
                        nome_tribunal, NOME_DO_GRUPO, "erro",
                        detalhe_erro=f"Erro ao captar processos: {erro_na_captacao}"
                    )
                else:
                    status_bots.atualizar_status(
                        nome_tribunal, NOME_DO_GRUPO, "rodando"
                    )
    ...
    # Saiu do while porque passou das 17h (ou é fim de semana) —
    # marca todos os tribunais como fora do horário antes de encerrar.
    print()
    print("Fora do horário permitido — encerrando a execução.")
    for tribunal in TRIBUNAIS:
        status_bots.atualizar_status(
            tribunal["nome"], NOME_DO_GRUPO, "fora_do_horario"
        )
